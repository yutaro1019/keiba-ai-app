...
| スタイル | 対象券種 | カバレッジ（LambdaRank v2） | カバレッジ（market） | ROI目安 |
|---|---|---|---|---|
| **Kelly AI** | 馬連・馬単 | **~88%** | — | **+43.9%** |
| **期待値重視** | 単勝・複勝・ワイド | **~100%** | ~31% | ~+8〜14% |
| **的中率重視** | 複勝・ワイド | **~100%** | ~82% | 参考値 |
| **自動選択** | 単勝・複勝・ワイド（自動） | **~100%** | ~82% | ~+8〜14% |

> カバレッジ = 買い目が1点以上発生したレースの割合。2024年3,454レース実測値。  
> LambdaRank v2 はオッズ特徴量を使わないため、市場オッズと独立した確率を出し、ほぼ全レースでEV+の買い目が見つかる。  
> Kelly AIはkellyバンクロール=100,000円・min_ev=10%・2024年Q1実測値。
